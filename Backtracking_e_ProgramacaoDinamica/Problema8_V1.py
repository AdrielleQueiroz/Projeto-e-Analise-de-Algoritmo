...
SHOW_SOLUTIONS = False #Flag para mostrar as soluções encontradas na tela
LIMIT_ADJACENT = 4 #Limite de células adjacentes a serem consideradas para restrição de cores
LIMIT_SOLUTIONS = 10**6
...
def check_adjacentes(i, j, matrix):
    ...
    for d in range(LIMIT_ADJACENT):
        if 0 <= i + X[d] < N and 0 <= j + Y[d] < N:  # está na matriz Super Infer
...
#Verifica possui cores validas principal e secundaria
...
# remove a configuração de uma submatriz 2x2
...
def solve(i, j, matrix):
    ...
    if i == N and j == 0:
        ...
        if numberSolutions == LIMIT_SOLUTIONS:
            print("Voce atingiu o limite de solucoes")
            print("programa foi encerrado pois pode demorar muito para processar!")
            sys.exit(0) # Encerra o programa ao atingir o limite de soluções
...
def menu():
    global N, K, SHOW_SOLUTIONS, LIMIT_SOLUTIONS, LIMIT_ADJACENT

    print("Digite a dimensão da matriz e o número de cores")
    N = int(input())
    K = int(input())

    opcao = ""
    while opcao not in ["s", "n"]:
        print("Você quer exibir as matrizes que satisfazem as condições? s/n (sim/não)")
    ...
    opcao = ""
    while opcao not in ["s", "n"]:
        print("Você quer limitar o número de matrizes que satisfazem as condições? s/n (sim/não)")
        opcao = input().strip().lower()
    if opcao == "s":
        print("Digite o limite de soluções:")
        LIMIT_SOLUTIONS = int(input())

    opcao = ""
    while opcao not in ["s", "n"]:
        print("Você quer considerar os vizinhos das diagonais?")
        opcao = input().strip().lower()
    if opcao == "s":
        LIMIT_ADJACENT = 8;
